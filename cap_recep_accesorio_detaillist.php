...
$cap_recep_accesorio_detail_list = NULL; // Initialize page object first

class ccap_recep_accesorio_detail_list extends ccap_recep_accesorio_detail {
	...
	// Page object name
	var $PageObjName = 'cap_recep_accesorio_detail_list';
	...
	function __construct() {
		...
		$this->AddUrl = "cap_recep_accesorio_detailadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "cap_recep_accesorio_detaildelete.php";
		$this->MultiUpdateUrl = "cap_recep_accesorio_detailupdate.php";
	...
	function RenderListOptions() {
		...
		if (($this->CurrentAction == "add" || $this->CurrentAction == "copy") && $this->RowType == EW_ROWTYPE_ADD) { // Inline Add/Copy
			...
			$oListOpt->Body = "<div" . (($oListOpt->OnLeft) ? " style=\"text-align: right\"" : "") . ">" .
				"<a class=\"ewGridLink\" href=\"\" onclick=\"return ewForms['fcap_recep_accesorio_detaillist'].Submit();\">" . "<img src=\"phpimages/insert.gif\" alt=\"" . ew_HtmlEncode($Language->Phrase("InsertLink")) . "\" title=\"" . ew_HtmlEncode($Language->Phrase("InsertLink")) . "\" width=\"16\" height=\"16\" style=\"border: 0;\">" . "</a>&nbsp;" .
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_cap_recep_accesorio_detail\" href=\"javascript:void(0);\" onclick=\"ew_EmailDialogShow({lnk:'emf_cap_recep_accesorio_detail',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fcap_recep_accesorio_detaillist,sel:false});\">" . "<img src=\"phpimages/exportemail.gif\" alt=\"" . ew_HtmlEncode($Language->Phrase("ExportToEmail")) . "\" title=\"" . ew_HtmlEncode($Language->Phrase("ExportToEmail")) . "\" width=\"16\" height=\"16\" style=\"border: 0;\">" . "</a>";
		$item->Visible = FALSE;
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "cap_recep_accesorio") {
			global $cap_recep_accesorio;
			$rsmaster = $cap_recep_accesorio->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $ExportDoc->Style;
				$ExportDoc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$cap_recep_accesorio->ExportDocument($ExportDoc, $rsmaster, 1, 1);
	...
	function SetUpMasterParms() {
		...
		if (isset($_GET[EW_TABLE_SHOW_MASTER])) {
			...
			if ($sMasterTblVar == "cap_recep_accesorio") {
				$bValidMaster = TRUE;
				if (@$_GET["Id_Compra"] <> "") {
					$GLOBALS["cap_recep_accesorio"]->Id_Compra->setQueryStringValue($_GET["Id_Compra"]);
					$this->Id_Compra->setQueryStringValue($GLOBALS["cap_recep_accesorio"]->Id_Compra->QueryStringValue);
					$this->Id_Compra->setSessionValue($this->Id_Compra->QueryStringValue);
					if (!is_numeric($GLOBALS["cap_recep_accesorio"]->Id_Compra->QueryStringValue)) $bValidMaster = FALSE;
				} else {
					$bValidMaster = FALSE;
				}
			}
...
// Create page object
if (!isset($cap_recep_accesorio_detail_list)) $cap_recep_accesorio_detail_list = new ccap_recep_accesorio_detail_list();

// Page init
$cap_recep_accesorio_detail_list->Page_Init();

// Page main
$cap_recep_accesorio_detail_list->Page_Main();
?>
...
// Page object
var cap_recep_accesorio_detail_list = new ew_Page("cap_recep_accesorio_detail_list");
cap_recep_accesorio_detail_list.PageID = "list"; // Page ID
var EW_PAGE_ID = cap_recep_accesorio_detail_list.PageID; // For backward compatibility

// Form object
var fcap_recep_accesorio_detaillist = new ew_Form("fcap_recep_accesorio_detaillist");

// Validate form
fcap_recep_accesorio_detaillist.Validate = function(fobj) {
...
fcap_recep_accesorio_detaillist.Form_CustomValidate = 
...
fcap_recep_accesorio_detaillist.ValidateRequired = true;
<?php } else { ?>
fcap_recep_accesorio_detaillist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcap_recep_accesorio_detaillist.Lists["x_COD_Fam_Accesorio"] = {"LinkField":"x_COD_Fam_Accesorio","Ajax":true,"AutoFill":false,"DisplayFields":["x_Familia_Accesorio","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcap_recep_accesorio_detaillist.Lists["x_Id_Articulo"] = {"LinkField":"x_Id_Articulo","Ajax":true,"AutoFill":true,"DisplayFields":["x_Articulo","","",""],"ParentFields":["x_COD_Fam_Accesorio"],"FilterFields":["x_COD_Fam_Accesorio"],"Options":[]};
...
<?php } ?>
<?php if (($cap_recep_accesorio_detail->Export == "") || (EW_EXPORT_MASTER_RECORD && $cap_recep_accesorio_detail->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "cap_recep_accesoriolist.php";
if ($cap_recep_accesorio_detail_list->DbMasterFilter <> "" && $cap_recep_accesorio_detail->getCurrentMasterTable() == "cap_recep_accesorio") {
	if ($cap_recep_accesorio_detail_list->MasterRecordExists) {
		if ($cap_recep_accesorio_detail->getCurrentMasterTable() == $cap_recep_accesorio_detail->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<p><span class="ewTitle ewMasterTableTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $cap_recep_accesorio->TableCaption() ?>&nbsp;&nbsp;</span><?php $cap_recep_accesorio_detail_list->ExportOptions->Render("body"); ?>
</p>
<?php if ($cap_recep_accesorio_detail->Export == "") { ?>
<p class="phpmaker"><a href="<?php echo $gsMasterReturnUrl ?>"><?php echo $Language->Phrase("BackToMasterRecordPage") ?></a></p>
<?php } ?>
<?php include_once "cap_recep_accesoriomaster.php" ?>
<?php
	}
}
?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$cap_recep_accesorio_detail_list->TotalRecs = $cap_recep_accesorio_detail->SelectRecordCount();
	} else {
		if ($cap_recep_accesorio_detail_list->Recordset = $cap_recep_accesorio_detail_list->LoadRecordset())
			$cap_recep_accesorio_detail_list->TotalRecs = $cap_recep_accesorio_detail_list->Recordset->RecordCount();
	}
	$cap_recep_accesorio_detail_list->StartRec = 1;
	if ($cap_recep_accesorio_detail_list->DisplayRecs <= 0 || ($cap_recep_accesorio_detail->Export <> "" && $cap_recep_accesorio_detail->ExportAll)) // Display all records
		$cap_recep_accesorio_detail_list->DisplayRecs = $cap_recep_accesorio_detail_list->TotalRecs;
	if (!($cap_recep_accesorio_detail->Export <> "" && $cap_recep_accesorio_detail->ExportAll))
		$cap_recep_accesorio_detail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$cap_recep_accesorio_detail_list->Recordset = $cap_recep_accesorio_detail_list->LoadRecordset($cap_recep_accesorio_detail_list->StartRec-1, $cap_recep_accesorio_detail_list->DisplayRecs);
?>
<p style="white-space: nowrap;"><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("TblTypeVIEW") ?><?php echo $cap_recep_accesorio_detail->TableCaption() ?>&nbsp;&nbsp;</span>
<?php if ($cap_recep_accesorio_detail->getCurrentMasterTable() == "") { ?>
<?php $cap_recep_accesorio_detail_list->ExportOptions->Render("body"); ?>
<?php } ?>
</p>
<?php $cap_recep_accesorio_detail_list->ShowPageHeader(); ?>
<?php
$cap_recep_accesorio_detail_list->ShowMessage();
?>
<br>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<?php if ($cap_recep_accesorio_detail->Export == "") { ?>
<div class="ewGridUpperPanel">
<?php if ($cap_recep_accesorio_detail->CurrentAction <> "gridadd" && $cap_recep_accesorio_detail->CurrentAction <> "gridedit") { ?>
<form name="ewpagerform" id="ewpagerform" class="ewForm" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager"><tr><td>
<?php if (!isset($cap_recep_accesorio_detail_list->Pager)) $cap_recep_accesorio_detail_list->Pager = new cPrevNextPager($cap_recep_accesorio_detail_list->StartRec, $cap_recep_accesorio_detail_list->DisplayRecs, $cap_recep_accesorio_detail_list->TotalRecs) ?>
<?php if ($cap_recep_accesorio_detail_list->Pager->RecordCount > 0) { ?>
	<table cellspacing="0" class="ewStdTable"><tbody><tr><td><span class="phpmaker"><?php echo $Language->Phrase("Page") ?>&nbsp;</span></td>
<!--first page button-->
	<?php if ($cap_recep_accesorio_detail_list->Pager->FirstButton->Enabled) { ?>
	<td><a href="<?php echo $cap_recep_accesorio_detail_list->PageUrl() ?>start=<?php echo $cap_recep_accesorio_detail_list->Pager->FirstButton->Start ?>"><img src="phpimages/first.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" style="border: 0;"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/firstdisab.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($cap_recep_accesorio_detail_list->Pager->PrevButton->Enabled) { ?>
	<td><a href="<?php echo $cap_recep_accesorio_detail_list->PageUrl() ?>start=<?php echo $cap_recep_accesorio_detail_list->Pager->PrevButton->Start ?>"><img src="phpimages/prev.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" style="border: 0;"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/prevdisab.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--current page number-->
	<td><input type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" id="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cap_recep_accesorio_detail_list->Pager->CurrentPage ?>" size="4"></td>
<!--next page button-->
	<?php if ($cap_recep_accesorio_detail_list->Pager->NextButton->Enabled) { ?>
	<td><a href="<?php echo $cap_recep_accesorio_detail_list->PageUrl() ?>start=<?php echo $cap_recep_accesorio_detail_list->Pager->NextButton->Start ?>"><img src="phpimages/next.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" style="border: 0;"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/nextdisab.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($cap_recep_accesorio_detail_list->Pager->LastButton->Enabled) { ?>
	<td><a href="<?php echo $cap_recep_accesorio_detail_list->PageUrl() ?>start=<?php echo $cap_recep_accesorio_detail_list->Pager->LastButton->Start ?>"><img src="phpimages/last.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" style="border: 0;"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/lastdisab.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" style="border: 0;"></td>
	<?php } ?>
	<td><span class="phpmaker">&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cap_recep_accesorio_detail_list->Pager->PageCount ?></span></td>
	</tr></tbody></table>
	</td>	
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td>
	<span class="phpmaker"><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $cap_recep_accesorio_detail_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $cap_recep_accesorio_detail_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $cap_recep_accesorio_detail_list->Pager->RecordCount ?></span>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($cap_recep_accesorio_detail_list->SearchWhere == "0=101") { ?>
...
<?php if ($cap_recep_accesorio_detail_list->TotalRecs > 0) { ?>
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<option value="20"<?php if ($cap_recep_accesorio_detail_list->DisplayRecs == 20) { ?> selected="selected"<?php } ?>>20</option>
<option value="40"<?php if ($cap_recep_accesorio_detail_list->DisplayRecs == 40) { ?> selected="selected"<?php } ?>>40</option>
<option value="60"<?php if ($cap_recep_accesorio_detail_list->DisplayRecs == 60) { ?> selected="selected"<?php } ?>>60</option>
<option value="100"<?php if ($cap_recep_accesorio_detail_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
<option value="ALL"<?php if ($cap_recep_accesorio_detail->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
...
<?php if ($cap_recep_accesorio_detail_list->InlineAddUrl <> "") { ?>
<a class="ewGridLink" href="<?php echo $cap_recep_accesorio_detail_list->InlineAddUrl ?>"><?php echo $Language->Phrase("InlineAddLink") ?></a>&nbsp;&nbsp;
<?php } ?>
<?php } ?>
</span>
</div>
<?php } ?>
<form name="fcap_recep_accesorio_detaillist" id="fcap_recep_accesorio_detaillist" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="cap_recep_accesorio_detail">
<div id="gmp_cap_recep_accesorio_detail" class="ewGridMiddlePanel">
<?php if ($cap_recep_accesorio_detail_list->TotalRecs > 0 || $cap_recep_accesorio_detail->CurrentAction == "add" || $cap_recep_accesorio_detail->CurrentAction == "copy") { ?>
<table id="tbl_cap_recep_accesorio_detaillist" class="ewTable ewTableSeparate">
...
// Render list options
$cap_recep_accesorio_detail_list->RenderListOptions();

// Render list options (header, left)
$cap_recep_accesorio_detail_list->ListOptions->Render("header", "left");
?>
<?php if ($cap_recep_accesorio_detail->COD_Fam_Accesorio->Visible) { // COD_Fam_Accesorio ?>
	...
	<?php } else { ?>
		<td><div onmousedown="ew_Sort(event,'<?php echo $cap_recep_accesorio_detail->SortUrl($cap_recep_accesorio_detail->COD_Fam_Accesorio) ?>',1);"><span id="elh_cap_recep_accesorio_detail_COD_Fam_Accesorio" class="cap_recep_accesorio_detail_COD_Fam_Accesorio">
...
<?php if ($cap_recep_accesorio_detail->Id_Articulo->Visible) { // Id_Articulo ?>
	...
	<?php } else { ?>
		<td><div onmousedown="ew_Sort(event,'<?php echo $cap_recep_accesorio_detail->SortUrl($cap_recep_accesorio_detail->Id_Articulo) ?>',1);"><span id="elh_cap_recep_accesorio_detail_Id_Articulo" class="cap_recep_accesorio_detail_Id_Articulo">
...
<?php if ($cap_recep_accesorio_detail->CantRecibida->Visible) { // CantRecibida ?>
	...
	<?php } else { ?>
		<td><div onmousedown="ew_Sort(event,'<?php echo $cap_recep_accesorio_detail->SortUrl($cap_recep_accesorio_detail->CantRecibida) ?>',1);"><span id="elh_cap_recep_accesorio_detail_CantRecibida" class="cap_recep_accesorio_detail_CantRecibida">
...
<?php if ($cap_recep_accesorio_detail->Precio_Unitario->Visible) { // Precio_Unitario ?>
	...
	<?php } else { ?>
		<td><div onmousedown="ew_Sort(event,'<?php echo $cap_recep_accesorio_detail->SortUrl($cap_recep_accesorio_detail->Precio_Unitario) ?>',1);"><span id="elh_cap_recep_accesorio_detail_Precio_Unitario" class="cap_recep_accesorio_detail_Precio_Unitario">
...
<?php if ($cap_recep_accesorio_detail->MontoTotal->Visible) { // MontoTotal ?>
	...
	<?php } else { ?>
		<td><div onmousedown="ew_Sort(event,'<?php echo $cap_recep_accesorio_detail->SortUrl($cap_recep_accesorio_detail->MontoTotal) ?>',1);"><span id="elh_cap_recep_accesorio_detail_MontoTotal" class="cap_recep_accesorio_detail_MontoTotal">
...
$cap_recep_accesorio_detail_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
	if ($cap_recep_accesorio_detail->CurrentAction == "add" || $cap_recep_accesorio_detail->CurrentAction == "copy") {
		$cap_recep_accesorio_detail_list->RowIndex = 0;
		$cap_recep_accesorio_detail_list->KeyCount = $cap_recep_accesorio_detail_list->RowIndex;
		if ($cap_recep_accesorio_detail->CurrentAction == "copy" && !$cap_recep_accesorio_detail_list->LoadRow())
				$cap_recep_accesorio_detail->CurrentAction = "add";
		if ($cap_recep_accesorio_detail->CurrentAction == "add")
			$cap_recep_accesorio_detail_list->LoadDefaultValues();
		if ($cap_recep_accesorio_detail->EventCancelled) // Insert failed
			$cap_recep_accesorio_detail_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$cap_recep_accesorio_detail->ResetAttrs();
		$cap_recep_accesorio_detail->RowAttrs = array_merge($cap_recep_accesorio_detail->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_cap_recep_accesorio_detail', 'data-rowtype'=>EW_ROWTYPE_ADD));
		$cap_recep_accesorio_detail->RowType = EW_ROWTYPE_ADD;

		// Render row
		$cap_recep_accesorio_detail_list->RenderRow();

		// Render list options
		$cap_recep_accesorio_detail_list->RenderListOptions();
		$cap_recep_accesorio_detail_list->StartRowCnt = 0;
?>
	<tr<?php echo $cap_recep_accesorio_detail->RowAttributes() ?>>
<?php

// Render list options (body, left)
$cap_recep_accesorio_detail_list->ListOptions->Render("body", "left", $cap_recep_accesorio_detail_list->RowCnt);
?>
	<?php if ($cap_recep_accesorio_detail->COD_Fam_Accesorio->Visible) { // COD_Fam_Accesorio ?>
		<td><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_COD_Fam_Accesorio" class="cap_recep_accesorio_detail_COD_Fam_Accesorio">
<?php $cap_recep_accesorio_detail->COD_Fam_Accesorio->EditAttrs["onchange"] = "ew_UpdateOpt.call(this, ['x" . $cap_recep_accesorio_detail_list->RowIndex . "_Id_Articulo']); " . @$cap_recep_accesorio_detail->COD_Fam_Accesorio->EditAttrs["onchange"]; ?>
<select id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio"<?php echo $cap_recep_accesorio_detail->COD_Fam_Accesorio->EditAttributes() ?>>
<?php
...
$sSqlWrk .= " ORDER BY `Familia_Accesorio` ASC";
?>
<input type="hidden" name="s_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" id="s_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" value="s=<?php echo TEAencrypt($sSqlWrk) ?>&fn=<?php echo urlencode($cap_recep_accesorio_detail->COD_Fam_Accesorio->LookupFn) ?>&f0=<?php echo TEAencrypt("`COD_Fam_Accesorio` = {filter_value}"); ?>&t0=200">
<input type="hidden" name="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" id="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->COD_Fam_Accesorio->OldValue) ?>">
</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Id_Articulo->Visible) { // Id_Articulo ?>
		<td><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_Id_Articulo" class="cap_recep_accesorio_detail_Id_Articulo">
...
<select id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo"<?php echo $cap_recep_accesorio_detail->Id_Articulo->EditAttributes() ?>>
<?php
...
$sSqlWrk .= " ORDER BY `Articulo` Asc";
?>
<input type="hidden" name="s_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" id="s_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" value="s=<?php echo TEAencrypt($sSqlWrk) ?>&fn=<?php echo urlencode($cap_recep_accesorio_detail->Id_Articulo->LookupFn) ?>&f0=<?php echo TEAencrypt("`Id_Articulo` = {filter_value}"); ?>&t0=3&f1=<?php echo TEAencrypt("`COD_Fam_Accesorio` IN ({filter_value})"); ?>&t1=200">
<?php
...
?>
<input type="hidden" name="sf_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" id="sf_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" value="<?php echo $sSqlWrk ?>">
<input type="hidden" name="ln_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" id="ln_x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" value="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario">
<input type="hidden" name="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" id="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->Id_Articulo->OldValue) ?>">
</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->CantRecibida->Visible) { // CantRecibida ?>
		<td><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_CantRecibida" class="cap_recep_accesorio_detail_CantRecibida">
<input type="text" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" size="5" value="<?php echo $cap_recep_accesorio_detail->CantRecibida->EditValue ?>"<?php echo $cap_recep_accesorio_detail->CantRecibida->EditAttributes() ?>>
<input type="hidden" name="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" id="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->CantRecibida->OldValue) ?>">
</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Precio_Unitario->Visible) { // Precio_Unitario ?>
		<td><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_Precio_Unitario" class="cap_recep_accesorio_detail_Precio_Unitario">
<input type="text" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" size="8" value="<?php echo $cap_recep_accesorio_detail->Precio_Unitario->EditValue ?>"<?php echo $cap_recep_accesorio_detail->Precio_Unitario->EditAttributes() ?>>
<input type="hidden" name="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" id="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->Precio_Unitario->OldValue) ?>">
</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->MontoTotal->Visible) { // MontoTotal ?>
		<td><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_MontoTotal" class="cap_recep_accesorio_detail_MontoTotal">
<input type="text" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" size="30" value="<?php echo $cap_recep_accesorio_detail->MontoTotal->EditValue ?>"<?php echo $cap_recep_accesorio_detail->MontoTotal->EditAttributes() ?>>
<input type="hidden" name="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" id="o<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->MontoTotal->OldValue) ?>">
</span></td>
	<?php } ?>
<?php

// Render list options (body, right)
$cap_recep_accesorio_detail_list->ListOptions->Render("body", "right", $cap_recep_accesorio_detail_list->RowCnt);
?>
<script type="text/javascript">
fcap_recep_accesorio_detaillist.UpdateOpts(<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>);
</script>
	</tr>
<?php
}
?>
<?php
if ($cap_recep_accesorio_detail->ExportAll && $cap_recep_accesorio_detail->Export <> "") {
	$cap_recep_accesorio_detail_list->StopRec = $cap_recep_accesorio_detail_list->TotalRecs;
} else {

	// Set the last record to display
	if ($cap_recep_accesorio_detail_list->TotalRecs > $cap_recep_accesorio_detail_list->StartRec + $cap_recep_accesorio_detail_list->DisplayRecs - 1)
		$cap_recep_accesorio_detail_list->StopRec = $cap_recep_accesorio_detail_list->StartRec + $cap_recep_accesorio_detail_list->DisplayRecs - 1;
	else
		$cap_recep_accesorio_detail_list->StopRec = $cap_recep_accesorio_detail_list->TotalRecs;
}
...
if ($objForm) {
	...
	if ($objForm->HasValue("key_count") && ($cap_recep_accesorio_detail->CurrentAction == "gridadd" || $cap_recep_accesorio_detail->CurrentAction == "gridedit" || $cap_recep_accesorio_detail->CurrentAction == "F")) {
		$cap_recep_accesorio_detail_list->KeyCount = $objForm->GetValue("key_count");
		$cap_recep_accesorio_detail_list->StopRec = $cap_recep_accesorio_detail_list->KeyCount;
	}
}
$cap_recep_accesorio_detail_list->RecCnt = $cap_recep_accesorio_detail_list->StartRec - 1;
if ($cap_recep_accesorio_detail_list->Recordset && !$cap_recep_accesorio_detail_list->Recordset->EOF) {
	$cap_recep_accesorio_detail_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $cap_recep_accesorio_detail_list->StartRec > 1)
		$cap_recep_accesorio_detail_list->Recordset->Move($cap_recep_accesorio_detail_list->StartRec - 1);
} elseif (!$cap_recep_accesorio_detail->AllowAddDeleteRow && $cap_recep_accesorio_detail_list->StopRec == 0) {
	$cap_recep_accesorio_detail_list->StopRec = $cap_recep_accesorio_detail->GridAddRowCount;
}
...
$cap_recep_accesorio_detail->ResetAttrs();
$cap_recep_accesorio_detail_list->RenderRow();
$cap_recep_accesorio_detail_list->EditRowCnt = 0;
if ($cap_recep_accesorio_detail->CurrentAction == "edit")
	$cap_recep_accesorio_detail_list->RowIndex = 1;
while ($cap_recep_accesorio_detail_list->RecCnt < $cap_recep_accesorio_detail_list->StopRec) {
	$cap_recep_accesorio_detail_list->RecCnt++;
	if (intval($cap_recep_accesorio_detail_list->RecCnt) >= intval($cap_recep_accesorio_detail_list->StartRec)) {
		$cap_recep_accesorio_detail_list->RowCnt++;
		...
		if ($cap_recep_accesorio_detail->CurrentAction == "gridadd") {
			$cap_recep_accesorio_detail_list->LoadDefaultValues(); // Load default values
		} else {
			$cap_recep_accesorio_detail_list->LoadRowValues($cap_recep_accesorio_detail_list->Recordset); // Load row values
		}
		$cap_recep_accesorio_detail->RowType = EW_ROWTYPE_VIEW; // Render view
		if ($cap_recep_accesorio_detail->CurrentAction == "edit") {
			if ($cap_recep_accesorio_detail_list->CheckInlineEditKey() && $cap_recep_accesorio_detail_list->EditRowCnt == 0) { // Inline edit
				$cap_recep_accesorio_detail->RowType = EW_ROWTYPE_EDIT; // Render edit
			}
		}
		if ($cap_recep_accesorio_detail->CurrentAction == "edit" && $cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT && $cap_recep_accesorio_detail->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$cap_recep_accesorio_detail_list->RestoreFormValues(); // Restore form values
		}
		if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT) // Edit row
			$cap_recep_accesorio_detail_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$cap_recep_accesorio_detail->RowAttrs = array_merge($cap_recep_accesorio_detail->RowAttrs, array('data-rowindex'=>$cap_recep_accesorio_detail_list->RowCnt, 'id'=>'r' . $cap_recep_accesorio_detail_list->RowCnt . '_cap_recep_accesorio_detail', 'data-rowtype'=>$cap_recep_accesorio_detail->RowType));
...
?>
	<?php if ($cap_recep_accesorio_detail->COD_Fam_Accesorio->Visible) { // COD_Fam_Accesorio ?>
		<td<?php echo $cap_recep_accesorio_detail->COD_Fam_Accesorio->CellAttributes() ?>><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_COD_Fam_Accesorio" class="cap_recep_accesorio_detail_COD_Fam_Accesorio">
...
<input type="hidden" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_COD_Fam_Accesorio" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->COD_Fam_Accesorio->CurrentValue) ?>">
<?php } ?>
...
<?php } ?>
</span><a id="<?php echo $cap_recep_accesorio_detail_list->PageObjName . "_row_" . $cap_recep_accesorio_detail_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT || $cap_recep_accesorio_detail->CurrentMode == "edit") { ?>
<input type="hidden" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Compra_Det" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Compra_Det" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->Id_Compra_Det->CurrentValue) ?>">
<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Id_Articulo->Visible) { // Id_Articulo ?>
		<td<?php echo $cap_recep_accesorio_detail->Id_Articulo->CellAttributes() ?>><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_Id_Articulo" class="cap_recep_accesorio_detail_Id_Articulo">
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<?php $cap_recep_accesorio_detail->Id_Articulo->EditAttrs["onchange"] = "ew_AutoFill(this); " . @$cap_recep_accesorio_detail->Id_Articulo->EditAttrs["onchange"]; ?>
<select id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Id_Articulo"<?php echo $cap_recep_accesorio_detail->Id_Articulo->EditAttributes() ?>>
<?php
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->CantRecibida->Visible) { // CantRecibida ?>
		<td<?php echo $cap_recep_accesorio_detail->CantRecibida->CellAttributes() ?>><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_CantRecibida" class="cap_recep_accesorio_detail_CantRecibida">
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<input type="text" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_CantRecibida" size="5" value="<?php echo $cap_recep_accesorio_detail->CantRecibida->EditValue ?>"<?php echo $cap_recep_accesorio_detail->CantRecibida->EditAttributes() ?>>
<?php } ?>
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Precio_Unitario->Visible) { // Precio_Unitario ?>
		<td<?php echo $cap_recep_accesorio_detail->Precio_Unitario->CellAttributes() ?>><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_Precio_Unitario" class="cap_recep_accesorio_detail_Precio_Unitario">
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<input type="text" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_Precio_Unitario" size="8" value="<?php echo $cap_recep_accesorio_detail->Precio_Unitario->EditValue ?>"<?php echo $cap_recep_accesorio_detail->Precio_Unitario->EditAttributes() ?>>
<?php } ?>
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
...
<?php } ?>
	...
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->MontoTotal->Visible) { // MontoTotal ?>
		<td<?php echo $cap_recep_accesorio_detail->MontoTotal->CellAttributes() ?>><span id="el<?php echo $cap_recep_accesorio_detail_list->RowCnt ?>_cap_recep_accesorio_detail_MontoTotal" class="cap_recep_accesorio_detail_MontoTotal">
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
...
<input type="hidden" name="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" id="x<?php echo $cap_recep_accesorio_detail_list->RowIndex ?>_MontoTotal" value="<?php echo ew_HtmlEncode($cap_recep_accesorio_detail->MontoTotal->CurrentValue) ?>">
<?php } ?>
<?php if ($cap_recep_accesorio_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span<?php echo $cap_recep_accesorio_detail->MontoTotal->ViewAttributes() ?>>
...
<?php
	...
	if ($cap_recep_accesorio_detail->CurrentAction <> "gridadd")
		$cap_recep_accesorio_detail_list->Recordset->MoveNext();
}
?>
</tbody>
<?php
...
$cap_recep_accesorio_detail->RowType = EW_ROWTYPE_AGGREGATE;
$cap_recep_accesorio_detail->ResetAttrs();
$cap_recep_accesorio_detail_list->RenderRow();
?>
<?php if ($cap_recep_accesorio_detail_list->TotalRecs > 0 && ($cap_recep_accesorio_detail->CurrentAction <> "gridadd" && $cap_recep_accesorio_detail->CurrentAction <> "gridedit")) { ?>
<tfoot><!-- Table footer -->
...
$cap_recep_accesorio_detail_list->ListOptions->Render("footer", "left");
?>
	<?php if ($cap_recep_accesorio_detail->COD_Fam_Accesorio->Visible) { // COD_Fam_Accesorio ?>
		<td><span id="elf_cap_recep_accesorio_detail_COD_Fam_Accesorio" class="cap_recep_accesorio_detail_COD_Fam_Accesorio">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Id_Articulo->Visible) { // Id_Articulo ?>
		<td><span id="elf_cap_recep_accesorio_detail_Id_Articulo" class="cap_recep_accesorio_detail_Id_Articulo">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->CantRecibida->Visible) { // CantRecibida ?>
		<td><span id="elf_cap_recep_accesorio_detail_CantRecibida" class="cap_recep_accesorio_detail_CantRecibida">
<?php echo $Language->Phrase("TOTAL") ?>: 
<?php echo $cap_recep_accesorio_detail->CantRecibida->ViewValue ?>
		</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->Precio_Unitario->Visible) { // Precio_Unitario ?>
		<td><span id="elf_cap_recep_accesorio_detail_Precio_Unitario" class="cap_recep_accesorio_detail_Precio_Unitario">
		&nbsp;
		</span></td>
	<?php } ?>
	<?php if ($cap_recep_accesorio_detail->MontoTotal->Visible) { // MontoTotal ?>
		<td><span id="elf_cap_recep_accesorio_detail_MontoTotal" class="cap_recep_accesorio_detail_MontoTotal">
<?php echo $Language->Phrase("TOTAL") ?>: 
...
$cap_recep_accesorio_detail_list->ListOptions->Render("footer", "right");
?>
	</tr>
</tfoot>	
<?php } ?>
...
<?php } ?>
<?php if ($cap_recep_accesorio_detail->CurrentAction == "add" || $cap_recep_accesorio_detail->CurrentAction == "copy") { ?>
<input type="hidden" name="key_count" id="key_count" value="<?php echo $cap_recep_accesorio_detail_list->KeyCount ?>">
<?php } ?>
<?php if ($cap_recep_accesorio_detail->CurrentAction == "edit") { ?>
<input type="hidden" name="key_count" id="key_count" value="<?php echo $cap_recep_accesorio_detail_list->KeyCount ?>">
<?php } ?>
<?php if ($cap_recep_accesorio_detail->CurrentAction == "") { ?>
...
// Close recordset
if ($cap_recep_accesorio_detail_list->Recordset)
	$cap_recep_accesorio_detail_list->Recordset->Close();
?>
<?php if ($cap_recep_accesorio_detail_list->TotalRecs > 0) { ?>
<?php if ($cap_recep_accesorio_detail->Export == "") { ?>
...
<?php if ($cap_recep_accesorio_detail->Export == "") { ?>
<script type="text/javascript">
fcap_recep_accesorio_detaillist.Init();
</script>
<?php } ?>
<?php
$cap_recep_accesorio_detail_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($cap_recep_accesorio_detail->Export == "") { ?>
...
<?php
$cap_recep_accesorio_detail_list->Page_Terminate();
?>
