...
$cap_entrega_equipo_det_grid = NULL; // Initialize page object first

class ccap_entrega_equipo_det_grid extends ccap_entrega_equipo_det {
	...
	// Page object name
	var $PageObjName = 'cap_entrega_equipo_det_grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cap_entrega_equipo_det"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->Id_Traspaso_Det->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Num_IMEI") && $objForm->HasValue("o_Num_IMEI") && $this->Num_IMEI->CurrentValue <> $this->Num_IMEI->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Id_Tel_SIM") && $objForm->HasValue("o_Id_Tel_SIM") && $this->Id_Tel_SIM->CurrentValue <> $this->Id_Tel_SIM->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->Id_Tel_SIM->setOldValue($objForm->GetValue("o_Id_Tel_SIM"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->Id_Traspaso_Det->CurrentValue = strval($arKeys[0]); // Id_Traspaso_Det
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cap_entrega_equipo") {
			...
			if ($GLOBALS["cap_entrega_equipo"]->EventCancelled) $this->EventCancelled = TRUE;
		}
