...
$ca_proveedores_edit = NULL; // Initialize page object first

class cca_proveedores_edit extends cca_proveedores {
	...
	// Page object name
	var $PageObjName = 'ca_proveedores_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Proveedor->CurrentValue == "")
			$this->Page_Terminate("ca_proveedoreslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		$sFilter = $this->KeyFilter();
			...
			$sFilterChk = "(`RazonSocial` = '" . ew_AdjustSql($this->RazonSocial->CurrentValue) . "')";
			...
			$sFilterChk = "(`NombreContacto` = '" . ew_AdjustSql($this->NombreContacto->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$this->CurrentFilter = $sFilterChk;
			$sSqlChk = $this->SQL();
			$conn->raiseErrorFn = 'ew_ErrorFn';
		...
		if ($rs->EOF) {
			...
			// RazonSocial
			$this->RazonSocial->SetDbValueDef($rsnew, $this->RazonSocial->CurrentValue, NULL, $this->RazonSocial->ReadOnly);

			// RFC
			$this->RFC->SetDbValueDef($rsnew, $this->RFC->CurrentValue, NULL, $this->RFC->ReadOnly);

			// NombreContacto
			$this->NombreContacto->SetDbValueDef($rsnew, $this->NombreContacto->CurrentValue, NULL, $this->NombreContacto->ReadOnly);

			// CalleYNumero
			$this->CalleYNumero->SetDbValueDef($rsnew, $this->CalleYNumero->CurrentValue, NULL, $this->CalleYNumero->ReadOnly);
			...
			// NumCuenta
			$this->NumCuenta->SetDbValueDef($rsnew, $this->NumCuenta->CurrentValue, NULL, $this->NumCuenta->ReadOnly);

			// CLABE
			$this->CLABE->SetDbValueDef($rsnew, $this->CLABE->CurrentValue, NULL, $this->CLABE->ReadOnly);

			// Maneja_Papeleta
			$this->Maneja_Papeleta->SetDbValueDef($rsnew, $this->Maneja_Papeleta->CurrentValue, NULL, $this->Maneja_Papeleta->ReadOnly);
...
// Create page object
if (!isset($ca_proveedores_edit)) $ca_proveedores_edit = new cca_proveedores_edit();

// Page init
$ca_proveedores_edit->Page_Init();

// Page main
$ca_proveedores_edit->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ca_proveedores_edit = new ew_Page("ca_proveedores_edit");
ca_proveedores_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = ca_proveedores_edit.PageID; // For backward compatibility

// Form object
var fca_proveedoresedit = new ew_Form("fca_proveedoresedit");

// Validate form
fca_proveedoresedit.Validate = function(fobj) {
...
fca_proveedoresedit.Form_CustomValidate = 
...
fca_proveedoresedit.ValidateRequired = true;
<?php } else { ?>
fca_proveedoresedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fca_proveedoresedit.Lists["x_Id_Estado"] = {"LinkField":"x_Id_estado","Ajax":null,"AutoFill":false,"DisplayFields":["x_Estado","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ca_proveedores->TableCaption() ?></span></p>
...
<?php $ca_proveedores_edit->ShowPageHeader(); ?>
<?php
$ca_proveedores_edit->ShowMessage();
?>
<form name="fca_proveedoresedit" id="fca_proveedoresedit" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="ca_proveedores">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_ca_proveedoresedit" class="ewTable">
...
<script type="text/javascript">
fca_proveedoresedit.Lists["x_Id_Estado"].Options = <?php echo (is_array($ca_proveedores->Id_Estado->EditValue)) ? ew_ArrayToJson($ca_proveedores->Id_Estado->EditValue, 1) : "[]" ?>;
</script>
...
<input type="hidden" name="x_Id_Proveedor" id="x_Id_Proveedor" value="<?php echo ew_HtmlEncode($ca_proveedores->Id_Proveedor->CurrentValue) ?>">
<br>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<script type="text/javascript">
fca_proveedoresedit.Init();
</script>
<?php
$ca_proveedores_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ca_proveedores_edit->Page_Terminate();
?>
