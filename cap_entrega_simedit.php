...
$cap_entrega_sim_edit = NULL; // Initialize page object first

class ccap_entrega_sim_edit extends ccap_entrega_sim {
	...
	// Page object name
	var $PageObjName = 'cap_entrega_sim_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			$this->Page_Terminate("cap_entrega_simlist.php");
		}
	...
	function Page_Main() {
		...
		if ($this->Id_Traspaso->CurrentValue == "")
			$this->Page_Terminate("cap_entrega_simlist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("cap_entrega_simlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "cap_entrega_simview.php")
	...
	function ValidateForm() {
		...
		if ($this->getCurrentDetailTable() == "cap_entrega_sim_det" && $GLOBALS["cap_entrega_sim_det"]->DetailEdit) {
			if (!isset($GLOBALS["cap_entrega_sim_det_grid"])) $GLOBALS["cap_entrega_sim_det_grid"] = new ccap_entrega_sim_det_grid(); // get detail page object
			$GLOBALS["cap_entrega_sim_det_grid"]->ValidateGridForm();
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// TipoArticulo
			$this->TipoArticulo->SetDbValueDef($rsnew, SIM_Card(), NULL);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if ($this->getCurrentDetailTable() == "cap_entrega_sim_det" && $GLOBALS["cap_entrega_sim_det"]->DetailEdit) {
						if (!isset($GLOBALS["cap_entrega_sim_det_grid"])) $GLOBALS["cap_entrega_sim_det_grid"] = new ccap_entrega_sim_det_grid(); // get detail page object
						$EditRow = $GLOBALS["cap_entrega_sim_det_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			if ($sDetailTblVar == "cap_entrega_sim_det") {
				if (!isset($GLOBALS["cap_entrega_sim_det_grid"]))
					$GLOBALS["cap_entrega_sim_det_grid"] = new ccap_entrega_sim_det_grid;
				if ($GLOBALS["cap_entrega_sim_det_grid"]->DetailEdit) {
					$GLOBALS["cap_entrega_sim_det_grid"]->CurrentMode = "edit";
					$GLOBALS["cap_entrega_sim_det_grid"]->CurrentAction = "gridedit";

					// Save current master table to detail table
					$GLOBALS["cap_entrega_sim_det_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["cap_entrega_sim_det_grid"]->setStartRecordNumber(1);
					$GLOBALS["cap_entrega_sim_det_grid"]->Id_Traspaso->FldIsDetailKey = TRUE;
					$GLOBALS["cap_entrega_sim_det_grid"]->Id_Traspaso->CurrentValue = $this->Id_Traspaso->CurrentValue;
					$GLOBALS["cap_entrega_sim_det_grid"]->Id_Traspaso->setSessionValue($GLOBALS["cap_entrega_sim_det_grid"]->Id_Traspaso->CurrentValue);
				}
			}
		}
	}
...
// Create page object
if (!isset($cap_entrega_sim_edit)) $cap_entrega_sim_edit = new ccap_entrega_sim_edit();

// Page init
$cap_entrega_sim_edit->Page_Init();

// Page main
$cap_entrega_sim_edit->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var cap_entrega_sim_edit = new ew_Page("cap_entrega_sim_edit");
cap_entrega_sim_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = cap_entrega_sim_edit.PageID; // For backward compatibility

// Form object
var fcap_entrega_simedit = new ew_Form("fcap_entrega_simedit");

// Validate form
fcap_entrega_simedit.Validate = function(fobj) {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm))
			return ew_OnError(this, elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($cap_entrega_sim->Id_Almacen_Recibe->FldCaption()) ?>");
...
fcap_entrega_simedit.Form_CustomValidate = 
...
fcap_entrega_simedit.ValidateRequired = true;
<?php } else { ?>
fcap_entrega_simedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcap_entrega_simedit.Lists["x_Id_Almacen_Entrega"] = {"LinkField":"x_Id_Almacen","Ajax":true,"AutoFill":false,"DisplayFields":["x_Almacen","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcap_entrega_simedit.Lists["x_Id_Almacen_Recibe"] = {"LinkField":"x_Id_Almacen","Ajax":true,"AutoFill":false,"DisplayFields":["x_Almacen","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcap_entrega_simedit.Lists["x_Id_Empleado_Entrega"] = {"LinkField":"x_IdEmpleado","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeVIEW") ?><?php echo $cap_entrega_sim->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $cap_entrega_sim->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $cap_entrega_sim_edit->ShowPageHeader(); ?>
<?php
$cap_entrega_sim_edit->ShowMessage();
?>
<form name="fcap_entrega_simedit" id="fcap_entrega_simedit" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="cap_entrega_sim">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_cap_entrega_simedit" class="ewTable">
<?php if ($cap_entrega_sim->Id_Traspaso->Visible) { // Id_Traspaso ?>
	<tr id="r_Id_Traspaso"<?php echo $cap_entrega_sim->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_cap_entrega_sim_Id_Traspaso"><table class="ewTableHeaderBtn"><tr><td><?php echo $cap_entrega_sim->Id_Traspaso->FldCaption() ?></td></tr></table></span></td>
...
<?php echo $cap_entrega_sim->Id_Traspaso->EditValue ?></span>
<input type="hidden" name="x_Id_Traspaso" id="x_Id_Traspaso" value="<?php echo ew_HtmlEncode($cap_entrega_sim->Id_Traspaso->CurrentValue) ?>">
</span><?php echo $cap_entrega_sim->Id_Traspaso->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($cap_entrega_sim->Id_Almacen_Recibe->Visible) { // Id_Almacen_Recibe ?>
	<tr id="r_Id_Almacen_Recibe"<?php echo $cap_entrega_sim->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_cap_entrega_sim_Id_Almacen_Recibe"><table class="ewTableHeaderBtn"><tr><td><?php echo $cap_entrega_sim->Id_Almacen_Recibe->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></td></tr></table></span></td>
		<td<?php echo $cap_entrega_sim->Id_Almacen_Recibe->CellAttributes() ?>><span id="el_cap_entrega_sim_Id_Almacen_Recibe">
<select id="x_Id_Almacen_Recibe" name="x_Id_Almacen_Recibe"<?php echo $cap_entrega_sim->Id_Almacen_Recibe->EditAttributes() ?>>
<?php
if (is_array($cap_entrega_sim->Id_Almacen_Recibe->EditValue)) {
	$arwrk = $cap_entrega_sim->Id_Almacen_Recibe->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($cap_entrega_sim->Id_Almacen_Recibe->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
$sSqlWrk .= " ORDER BY `Almacen` Asc";
?>
<input type="hidden" name="s_x_Id_Almacen_Recibe" id="s_x_Id_Almacen_Recibe" value="s=<?php echo TEAencrypt($sSqlWrk) ?>&fn=<?php echo urlencode($cap_entrega_sim->Id_Almacen_Recibe->LookupFn) ?>&f0=<?php echo TEAencrypt("`Id_Almacen` = {filter_value}"); ?>&t0=19">
</span><?php echo $cap_entrega_sim->Id_Almacen_Recibe->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</div>
...
<br>
<?php if ($cap_entrega_sim->getCurrentDetailTable() == "cap_entrega_sim_det" && $cap_entrega_sim_det->DetailEdit) { ?>
<br>
<?php include_once "cap_entrega_sim_detgrid.php" ?>
<br>
<?php } ?>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<script type="text/javascript">
fcap_entrega_simedit.Init();
</script>
<?php
$cap_entrega_sim_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cap_entrega_sim_edit->Page_Terminate();
?>
