...
$cap_inventario_equipos_det_grid = NULL; // Initialize page object first

class ccap_inventario_equipos_det_grid extends ccap_inventario_equipos_det {
	...
	// Page object name
	var $PageObjName = 'cap_inventario_equipos_det_grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cap_inventario_equipos_det"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->Id_Almacen->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->Id_Articulo->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Articulo") && $objForm->HasValue("o_Articulo") && $this->Articulo->CurrentValue <> $this->Articulo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Acabado_eq") && $objForm->HasValue("o_Acabado_eq") && $this->Acabado_eq->CurrentValue <> $this->Acabado_eq->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('Id_Almacen');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('Id_Articulo');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->Id_Almacen->CurrentValue = strval($arKeys[0]); // Id_Almacen
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->Id_Articulo->CurrentValue = strval($arKeys[1]); // Id_Articulo
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Acabado_eq
			$this->Acabado_eq->SetDbValueDef($rsnew, $this->Acabado_eq->CurrentValue, "", $this->Acabado_eq->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "cap_inventario_equipos") {
				$this->Id_Almacen->CurrentValue = $this->Id_Almacen->getSessionValue();
		...
		if ($this->Id_Almacen->getSessionValue() <> "") {
			$rsnew['Id_Almacen'] = $this->Id_Almacen->getSessionValue();
		}

		// Id_Articulo
		if ($this->Id_Articulo->getSessionValue() <> "") {
			$rsnew['Id_Articulo'] = $this->Id_Articulo->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cap_inventario_equipos") {
			...
			if ($GLOBALS["cap_inventario_equipos"]->EventCancelled) $this->EventCancelled = TRUE;
