...
$cap_entrega_sim_add = NULL; // Initialize page object first

class ccap_entrega_sim_add extends ccap_entrega_sim {
	...
	// Page object name
	var $PageObjName = 'cap_entrega_sim_add';
	...
	function ValidateForm() {
		...
		if ($this->getCurrentDetailTable() == "cap_entrega_sim_det" && $GLOBALS["cap_entrega_sim_det"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			if ($this->getCurrentDetailTable() == "cap_entrega_sim_det" && $GLOBALS["cap_entrega_sim_det"]->DetailAdd) {
				$GLOBALS["cap_entrega_sim_det"]->Id_Traspaso->setSessionValue($this->Id_Traspaso->CurrentValue); // Set master key
				if (!isset($GLOBALS["cap_entrega_sim_det_grid"])) $GLOBALS["cap_entrega_sim_det_grid"] = new ccap_entrega_sim_det_grid(); // get detail page object
				$AddRow = $GLOBALS["cap_entrega_sim_det_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["cap_entrega_sim_det"]->Id_Traspaso->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			if ($sDetailTblVar == "cap_entrega_sim_det") {
				...
				if ($GLOBALS["cap_entrega_sim_det_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["cap_entrega_sim_det_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["cap_entrega_sim_det_grid"]->CurrentMode = "add";
					$GLOBALS["cap_entrega_sim_det_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($cap_entrega_sim_add)) $cap_entrega_sim_add = new ccap_entrega_sim_add();

// Page init
$cap_entrega_sim_add->Page_Init();

// Page main
$cap_entrega_sim_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var cap_entrega_sim_add = new ew_Page("cap_entrega_sim_add");
cap_entrega_sim_add.PageID = "add"; // Page ID
var EW_PAGE_ID = cap_entrega_sim_add.PageID; // For backward compatibility

// Form object
var fcap_entrega_simadd = new ew_Form("fcap_entrega_simadd");

// Validate form
fcap_entrega_simadd.Validate = function(fobj) {
...
fcap_entrega_simadd.Form_CustomValidate = 
...
fcap_entrega_simadd.ValidateRequired = true;
<?php } else { ?>
fcap_entrega_simadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcap_entrega_simadd.Lists["x_Id_Almacen_Entrega"] = {"LinkField":"x_Id_Almacen","Ajax":true,"AutoFill":false,"DisplayFields":["x_Almacen","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcap_entrega_simadd.Lists["x_Id_Almacen_Recibe"] = {"LinkField":"x_Id_Almacen","Ajax":true,"AutoFill":false,"DisplayFields":["x_Almacen","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcap_entrega_simadd.Lists["x_Id_Empleado_Entrega"] = {"LinkField":"x_IdEmpleado","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeVIEW") ?><?php echo $cap_entrega_sim->TableCaption() ?></span></p>
...
<?php $cap_entrega_sim_add->ShowPageHeader(); ?>
<?php
$cap_entrega_sim_add->ShowMessage();
?>
<form name="fcap_entrega_simadd" id="fcap_entrega_simadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="cap_entrega_sim">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_cap_entrega_simadd" class="ewTable">
...
<br>
<?php if ($cap_entrega_sim->getCurrentDetailTable() == "cap_entrega_sim_det" && $cap_entrega_sim_det->DetailAdd) { ?>
<br>
...
<script type="text/javascript">
fcap_entrega_simadd.Init();
</script>
<?php
$cap_entrega_sim_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cap_entrega_sim_add->Page_Terminate();
?>
