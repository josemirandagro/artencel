...
$ca_clientes_edit = NULL; // Initialize page object first

class cca_clientes_edit extends cca_clientes {
	...
	// Page object name
	var $PageObjName = 'ca_clientes_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Cliente->CurrentValue == "")
			$this->Page_Terminate("ca_clienteslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		$sFilter = $this->KeyFilter();
			...
			$sFilterChk = "(`Nombre_Completo` = '" . ew_AdjustSql($this->Nombre_Completo->CurrentValue) . "')";
			...
			$sFilterChk = "(`Razon_Social` = '" . ew_AdjustSql($this->Razon_Social->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$this->CurrentFilter = $sFilterChk;
			$sSqlChk = $this->SQL();
			$conn->raiseErrorFn = 'ew_ErrorFn';
			...
			$sFilterChk = "(`RFC` = '" . ew_AdjustSql($this->RFC->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$this->CurrentFilter = $sFilterChk;
			$sSqlChk = $this->SQL();
			$conn->raiseErrorFn = 'ew_ErrorFn';
			...
			$sFilterChk = "(`CURP` = '" . ew_AdjustSql($this->CURP->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$this->CurrentFilter = $sFilterChk;
			$sSqlChk = $this->SQL();
			$conn->raiseErrorFn = 'ew_ErrorFn';
		...
		if ($rs->EOF) {
			...
			// Nombre_Completo
			$this->Nombre_Completo->SetDbValueDef($rsnew, $this->Nombre_Completo->CurrentValue, NULL, $this->Nombre_Completo->ReadOnly);

			// Domicilio
			$this->Domicilio->SetDbValueDef($rsnew, $this->Domicilio->CurrentValue, NULL, $this->Domicilio->ReadOnly);

			// Num_Exterior
			$this->Num_Exterior->SetDbValueDef($rsnew, $this->Num_Exterior->CurrentValue, NULL, $this->Num_Exterior->ReadOnly);

			// Num_Interior
			$this->Num_Interior->SetDbValueDef($rsnew, $this->Num_Interior->CurrentValue, NULL, $this->Num_Interior->ReadOnly);

			// Colonia
			$this->Colonia->SetDbValueDef($rsnew, $this->Colonia->CurrentValue, NULL, $this->Colonia->ReadOnly);
			...
			// Id_Estado
			$this->Id_Estado->SetDbValueDef($rsnew, $this->Id_Estado->CurrentValue, NULL, $this->Id_Estado->ReadOnly);
			...
			// CURP
			$this->CURP->SetDbValueDef($rsnew, $this->CURP->CurrentValue, NULL, $this->CURP->ReadOnly);

			// Tel_Particular
			$this->Tel_Particular->SetDbValueDef($rsnew, $this->Tel_Particular->CurrentValue, NULL, $this->Tel_Particular->ReadOnly);

			// Tel_Oficina
			$this->Tel_Oficina->SetDbValueDef($rsnew, $this->Tel_Oficina->CurrentValue, NULL, $this->Tel_Oficina->ReadOnly);
			...
			// Tipo_Identificacion
			$this->Tipo_Identificacion->SetDbValueDef($rsnew, $this->Tipo_Identificacion->CurrentValue, NULL, $this->Tipo_Identificacion->ReadOnly);

			// Otra_Identificacion
			$this->Otra_Identificacion->SetDbValueDef($rsnew, $this->Otra_Identificacion->CurrentValue, NULL, $this->Otra_Identificacion->ReadOnly);

			// Numero_Identificacion
			$this->Numero_Identificacion->SetDbValueDef($rsnew, $this->Numero_Identificacion->CurrentValue, NULL, $this->Numero_Identificacion->ReadOnly);
...
// Create page object
if (!isset($ca_clientes_edit)) $ca_clientes_edit = new cca_clientes_edit();

// Page init
$ca_clientes_edit->Page_Init();

// Page main
$ca_clientes_edit->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ca_clientes_edit = new ew_Page("ca_clientes_edit");
ca_clientes_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = ca_clientes_edit.PageID; // For backward compatibility

// Form object
var fca_clientesedit = new ew_Form("fca_clientesedit");

// Validate form
fca_clientesedit.Validate = function(fobj) {
...
fca_clientesedit.Form_CustomValidate = 
...
fca_clientesedit.ValidateRequired = true;
<?php } else { ?>
fca_clientesedit.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
fca_clientesedit.MultiPage = new ew_MultiPage("fca_clientesedit");
fca_clientesedit.MultiPage.Elements = [["x_Nombre_Completo",1],["x_Domicilio",1],["x_Num_Exterior",1],["x_Num_Interior",1],["x_Colonia",1],["x_Poblacion",1],["x_MunicipioDel",1],["x_Id_Estado",1],["x_CP",1],["x_RFC",1],["x_Categoria",1],["x_CURP",2],["x_Tel_Particular",2],["x_Tel_Oficina",2],["x_Celular",2],["x_Edad",2],["x_Sexo",1],["x_Tipo_Identificacion",2],["x_Otra_Identificacion",2],["x_Numero_Identificacion",2],["x__EMail",2],["x_Comentarios",2]];

// Dynamic selection lists
fca_clientesedit.Lists["x_Id_Estado"] = {"LinkField":"x_Id_estado","Ajax":null,"AutoFill":false,"DisplayFields":["x_Estado","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ca_clientes->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $ca_clientes->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $ca_clientes_edit->ShowPageHeader(); ?>
<?php
$ca_clientes_edit->ShowMessage();
?>
<form name="fca_clientesedit" id="fca_clientesedit" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="ca_clientes">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewStdTable"><tbody><tr><td>
<div id="ca_clientes_edit" class="yui-navset">
	<ul class="yui-nav">
...
<table id="tbl_ca_clientesedit1" class="ewTable">
...
<script type="text/javascript">
fca_clientesedit.Lists["x_Id_Estado"].Options = <?php echo (is_array($ca_clientes->Id_Estado->EditValue)) ? ew_ArrayToJson($ca_clientes->Id_Estado->EditValue, 1) : "[]" ?>;
</script>
...
<table cellspacing="0" class="ewGrid" style="width: 100%"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_ca_clientesedit2" class="ewTable">
...
<input type="hidden" name="x_Id_Cliente" id="x_Id_Cliente" value="<?php echo ew_HtmlEncode($ca_clientes->Id_Cliente->CurrentValue) ?>">
<br>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo ew_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<script type="text/javascript">
fca_clientesedit.Init();
</script>
<?php
$ca_clientes_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ca_clientes_edit->Page_Terminate();
?>
