...
$cap_inventario_simcards_det_grid = NULL; // Initialize page object first

class ccap_inventario_simcards_det_grid extends ccap_inventario_simcards_det {
	...
	// Page object name
	var $PageObjName = 'cap_inventario_simcards_det_grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["cap_inventario_simcards_det"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Articulo") && $objForm->HasValue("o_Articulo") && $this->Articulo->CurrentValue <> $this->Articulo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Num_ICCID") && $objForm->HasValue("o_Num_ICCID") && $this->Num_ICCID->CurrentValue <> $this->Num_ICCID->OldValue)
			return FALSE;
	...
	function LoadFormValues() {
		...
		$this->Num_ICCID->setOldValue($objForm->GetValue("o_Num_ICCID"));
		if (!$this->Num_CEL->FldIsDetailKey) {
			$this->Num_CEL->setFormValue($objForm->GetValue("x_Num_CEL"));
		}
		$this->Num_CEL->setOldValue($objForm->GetValue("o_Num_CEL"));
	...
	function EditRow() {
		...
		$sFilter = $this->KeyFilter();
			...
			if ($this->Num_CEL->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`Num_CEL` = '" . ew_AdjustSql($this->Num_CEL->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$this->CurrentFilter = $sFilterChk;
			$sSqlChk = $this->SQL();
			$conn->raiseErrorFn = 'ew_ErrorFn';
			...
			if ($rsChk === FALSE) {
				...
				$sIdxErrMsg = str_replace("%f", $this->Num_CEL->FldCaption(), $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $this->Num_CEL->CurrentValue, $sIdxErrMsg);
				$this->setFailureMessage($sIdxErrMsg);
				$rsChk->Close();
				return FALSE;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->Num_CEL->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(Num_CEL = '" . ew_AdjustSql($this->Num_CEL->CurrentValue) . "')";
			$rsChk = $this->LoadRs($sFilter);
			if ($rsChk && !$rsChk->EOF) {
				$sIdxErrMsg = str_replace("%f", $this->Num_CEL->FldCaption(), $Language->Phrase("DupIndex"));
				$sIdxErrMsg = str_replace("%v", $this->Num_CEL->CurrentValue, $sIdxErrMsg);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cap_inventario_simcards") {
			...
			if ($GLOBALS["cap_inventario_simcards"]->EventCancelled) $this->EventCancelled = TRUE;
