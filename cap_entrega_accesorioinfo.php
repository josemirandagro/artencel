...
$cap_entrega_accesorio = NULL;

//
// Table class for cap_entrega_accesorio
//
class ccap_entrega_accesorio extends cTable {
	var $Id_Traspaso;
	...
	function __construct() {
		...
		$this->TableVar = 'cap_entrega_accesorio';
		$this->TableName = 'cap_entrega_accesorio';
		$this->TableType = 'VIEW';
		...
		// Id_Traspaso
		$this->Id_Traspaso = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Id_Traspaso', 'Id_Traspaso', '`Id_Traspaso`', '`Id_Traspaso`', 3, -1, FALSE, '`Id_Traspaso`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Id_Traspaso->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Id_Traspaso'] = &$this->Id_Traspaso;

		// Id_Almacen_Entrega
		$this->Id_Almacen_Entrega = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Id_Almacen_Entrega', 'Id_Almacen_Entrega', '`Id_Almacen_Entrega`', '`Id_Almacen_Entrega`', 3, -1, FALSE, '`Id_Almacen_Entrega`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// Id_Almacen_Recibe
		$this->Id_Almacen_Recibe = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Id_Almacen_Recibe', 'Id_Almacen_Recibe', '`Id_Almacen_Recibe`', '`Id_Almacen_Recibe`', 3, -1, FALSE, '`Id_Almacen_Recibe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// Fecha
		$this->Fecha = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Fecha', 'Fecha', '`Fecha`', 'DATE_FORMAT(`Fecha`, \'%d/%m/%Y %H:%i:%s\')', 133, 7, FALSE, '`Fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['Fecha'] = &$this->Fecha;

		// Hora
		$this->Hora = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Hora', 'Hora', '`Hora`', 'DATE_FORMAT(`Hora`, \'%d/%m/%Y %H:%i:%s\')', 134, -1, FALSE, '`Hora`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Hora->FldDefaultErrMsg = $Language->Phrase("IncorrectTime");
		$this->fields['Hora'] = &$this->Hora;

		// Status
		$this->Status = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_Status', 'Status', '`Status`', '`Status`', 202, -1, FALSE, '`Status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Status'] = &$this->Status;

		// TipoArticulo
		$this->TipoArticulo = new cField('cap_entrega_accesorio', 'cap_entrega_accesorio', 'x_TipoArticulo', 'TipoArticulo', '`TipoArticulo`', '`TipoArticulo`', 202, -1, FALSE, '`TipoArticulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "cap_entrega_accesorio_det") {
			$sDetailUrl = $GLOBALS["cap_entrega_accesorio_det"]->GetListUrl() . "?showmaster=" . $this->TableVar;
	...
	// Update Table
	var $UpdateTable = "`cap_entrega_accesorio`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "cap_entrega_accesoriolist.php";
		}
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("cap_entrega_accesorioview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "cap_entrega_accesorioadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("cap_entrega_accesorioedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("cap_entrega_accesorioedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("cap_entrega_accesorioadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("cap_entrega_accesorioadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cap_entrega_accesoriodelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function Row_Inserted($rsold, &$rsnew) {
	  ...
	  //  OJO, ME MANDA AL ULTIMO REGISTRO DE TRASPASO INSERTADO, HAY QUE LOCALIZAR LA COOKIE Y VER SI SE PUEDE MODIFICAR
	//  header( 'Location: cap_entrega_accesorio_detlist.php?a=add');              
	//  exit;                                 
	...
	//echo " y entragando desde el almacen " .$rsnew['Id_Almacen_Entrega'] ."<br>";     
	//echo " y recibiendo en el almacen " .$rsnew['Id_Almacen_Recibe'] ."<br>";     
	  ...
	  if  (($rsnew['Status']== "Enviado") or (($rsnew['Status']== "Recibido") and ($rsold['Status']== "Registrado")))  {           
		 ...
		 //  Actualizamos la Cantidad_MustBe en reg_existencias_accesorios   
		$AuxSQL= "UPDATE z_act_exist_trasp_acc set Cantidad_MustBe=Cantidad_MustBe-Cantidad                                   
					WHERE Id_Traspaso=". $rsold['Id_Traspaso'] . " AND Id_Almacen = ".$rsnew['Id_Almacen_Entrega'] ."" ;
	   $GLOBALS["conn"]->Execute($AuxSQL);                                                                            

		// Actualizamos el Status de Salida en los renglones traspaso_det
		$AuxSQL= "UPDATE z_act_exist_trasp_acc set Status='Enviado' 
	  ...
	  if  ($rsnew['Status']== "Recibido") {                                                 
		 ...
		 //  Actualizamos la Cantidad_MustBe en reg_existencias_accesorios   
		$AuxSQL= "UPDATE z_act_exist_trasp_acc set Cantidad_MustBe=Cantidad_MustBe+Cantidad                                   
				  WHERE Id_Traspaso=". $rsold['Id_Traspaso'] . " AND Id_Almacen = ".$rsnew['Id_Almacen_Recibe'] ."" ;
	   $GLOBALS["conn"]->Execute($AuxSQL);                                                                            

		// Actualizamos el Status de Salida en los renglones traspaso_det
		$AuxSQL= "UPDATE z_act_exist_trasp_acc set Status='Recibido'                                   
				  WHERE Id_Traspaso=". $rsold['Id_Traspaso'] ."" ;
	   $GLOBALS["conn"]->Execute($AuxSQL);                                                                            
	  }
